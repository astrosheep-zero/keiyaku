...
export const KEIYAKU_ARCHITECTURE_POLICY = {
  zones: [
    ...
    { source: "akuma/providers/acp/config.ts", allow: [] },
    ...
    },
    {
      ...
      allow: [
        ...
        any("library/configuration.ts", ["EMPTY_WORKTREE_HOOKS"]),
  ...
  forbiddenSourcePatterns: [
    {
      source: "cli/main.ts",
      pattern: /^import (?!type ).*from "\.\/(?:runtime|invoke)\.js"/mu,
      detail: "help completes at parse; selected family execution loads after a non-help parse",
    },
    {
      source: "cli/parse.ts",
      pattern: /^import (?!type ).*from "\.\.\/(?:index|task\/index|settings|kanshi\/index|akuma\/index)\.js"/mu,
      detail: "parse and help do not load package-root or product execution families",
    },
    {
      source: "cli/parse.ts",
      pattern: /^import (?!type ).*from "\.\/commands\/(?:akuma-invoke|task-invoke)\.js"/mu,
      detail: "parse and help do not load Task or Akuma command execution",
    },
    {
      source: "cli/commands/install.ts",
      pattern: /^import (?!type ).*from "\.\.\/\.\.\/runtime\/proc\/run\.js"/mu,
      detail: "install process runtime loads only when installing",
    },
    {
      source: "cli/commands/task-query.ts",
      pattern: /^import (?!type ).*from "\.\.\/\.\.\/task\/index\.js"/mu,
      detail: "task query parsing does not load Task product execution",
    },
    {
      source: "task/mutation.ts",
      pattern: /^import (?!type ).*from "\.\/index\.js"/mu,
      detail: "Task product execution loads only when executing a mutation",
    },
    {
      source: "cli/invoke.ts",
      pattern: /^import (?!type ).*from "\.\/commands\/(?:akuma-invoke|task-invoke|install)\.js"/mu,
      detail: "selected command execution enters invoke through a dynamic post-parse edge",
    },
    {
      source: "cli/invoke.ts",
      pattern: /^import (?!type ).*from "\.\.\/(?:index|kanshi\/index|settings|akuma\/index)\.js"/mu,
      detail: "Contract, Kanshi, Settings, and Akuma execution load only for commands that need them",
    },
    {
      source: "cli/runtime.ts",
      pattern: /^import (?!type ).*from "\.\/(?:invoke|commands\/(?:akuma-invoke|task-invoke|install))\.js"/mu,
      detail: "selected family execution enters runtime through a dynamic post-parse edge",
    },
    {
      source: "cli/runtime.ts",
      pattern: /^import (?!type ).*from "\.\.\/(?:index|kanshi\/index|settings|akuma\/index)\.js"/mu,
    ...
    },
    {
      source: "cli/coordinates.ts",
      pattern: /^import (?!type ).*from "\.\.\/index\.js"/mu,
      detail: "coordinate resolution does not load the package root",
    },
    {
    ...
    },
    {
      ...
      pattern: /^import (?!type ).*from "@anthropic-ai\/claude-agent-sdk"/mu,
      detail: "Claude adapter loads its native SDK only when starting or resuming",
    },
    {
      source: "akuma/providers/pi/index.ts",
      pattern: /^import (?!type ).*from "@earendil-works\/pi-coding-agent"/mu,
      detail: "Pi adapter loads its native SDK only when starting or resuming",
    },
    {
      source: "akuma/providers/opencode-sdk/**",
      pattern: /^import (?!type ).*from "@opencode-ai\/sdk"/mu,
      detail: "OpenCode adapter loads its native SDK only when starting or resuming",
    },

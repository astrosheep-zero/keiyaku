...
export const FILE_LINE_EXEMPTIONS = Object.freeze([
  owner("scripts/architecture/policy.ts", "The centralized declarative architecture map remains one policy authority; line compression had made active declarations unreadable.", 2130),
  ...
  owner("src/akuma/akuma.ts", "The low-level Akuma owner keeps birth, handles, lifecycle controls, and fleet reads coherent; splitting those operations would obscure that boundary.", 616),
  owner("src/library/contract.ts", "The package-root Contract handle remains one operations owner.", 763),
  owner("src/cli/invoke.ts", "CLI invocation and adaptation stay one command boundary.", 683, [functionOwner("invokeParsed", "Parsing has already established the one invocation boundary; command adaptation and execution stay together.", 85)]),
  owner("src/akuma/projection.ts", "The Akuma projection owns the one pure translation from retained Heart facts to public turn values; splitting it would duplicate projection ordering knowledge.", 531, [functionOwner("projectTurns", "Turn projection and its ordering and filtering rules are one read-time projection owner.", 86)]),
  ...
  owner("src/cli/parse.ts", "CLI grammar parsing is one public grammar owner consumed by invocation, runtime, selectors, and help rendering.", 526),

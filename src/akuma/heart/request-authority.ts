...
import { withHeartTransaction, withReadOnlyHeart } from "./storage.js";
...
export async function readRequest(paths: AkumaPaths, id: string): Promise<RequestFact | null> {
  return await withReadOnlyHeart(paths, (heart) => requestFact(heart, id));
}

export async function readNonterminalRequests(paths: AkumaPaths): Promise<readonly RequestFact[]> {
  return await withReadOnlyHeart(paths, nonterminalRequestFacts);
}

...
import { renderAccepted, renderFailed, renderRetry } from "./contract.js";
...
export function renderText(result: InvocationResult, context?: TextRenderContext): string {
  if (result.kind === "status") return renderKanshiText(result.report, context);
  if (result.kind === "accepted") return renderAccepted(result);
  if (result.kind === "failed") return renderFailed(result);

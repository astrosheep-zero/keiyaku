...
import type { ActorId, BindData, BoundData, ContractId, JournalEntry } from "../facts/types.js";
...
export function decideBind({ input, attempt, observation }: DecideInput<BindInput>): OfferDecision<BindRefusal> {
  ...
  if (input.data.terms.after.includes(id)) return { kind: "refused", refusal: { kind: "invalid-after", contractId: id } };
  ...
  const dependencies = input.data.terms.after;

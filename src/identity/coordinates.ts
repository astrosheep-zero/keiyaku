...
function prefix(family: IdentityFamily): string { return `${family}/`; }

function validSegment(segment: string): boolean {
  return segment.length > 0 && !/[\/\s\p{Cc}]/u.test(segment);
}

export function identityCoordinate(input: Readonly<{
...
export function identitySegments(input: Readonly<{
  family: IdentityFamily;
  value: string;
}>): readonly string[] {

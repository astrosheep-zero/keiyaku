...
test("model impact reports semantic field reach across owners without failing on fan-out", () => {
  const base = [
    ...
    source("src/core/facts/fold.ts", 'import type { ContractState } from "./types.js"; export function read(value: ContractState) { return value.delivery; }'),
  ];
  const head = [
    source("src/core/facts/types.ts", "export type ContractState = { delivery?: number }"),
    source("src/core/facts/fold.ts", [
    ...
    source("src/cli/invoke.ts", 'import type { ContractState } from "../core/facts/types.js"; export function render(value: ContractState) { const { delivery } = value; return delivery; }'),
  ];
  ...
  assert.deepEqual(new Set(report.fields[0]?.after?.usages.map((usage) => usage.kind)), new Set(["construct", "declaration", "destructure", "read", "write"]));
});

test("model impact resolves inherited and aliased exported model fields", () => {
  const base = [
    source("src/core/facts/types.ts", [
  ...
  const report = analyzeModelImpact(base, head, { base: "base", head: "head" }, MODEL_IMPACT_POLICY);
  assert.deepEqual(report.fields.map((field) => field.model), ["ContractAlias", "ContractState"]);

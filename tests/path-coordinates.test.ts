...
test("registered worktrees retain a prunable record below a regular-file parent", async () => {
  ...
  mkdirSync(parent);
  execFileSync("git", ["-C", path, "worktree", "add", "--quiet", "--detach", missing]);
  rmSync(parent, { recursive: true, force: true });
  writeFileSync(parent, "not a directory");

  const worktrees = await registeredWorktrees(await repositoryAt(path));
  const retained = worktrees.find((worktree) => worktree.path.endsWith(join("not-a-directory", "missing")));
  assert.notEqual(retained, undefined);
  assert.equal(retained?.branch, null);
});

...
test("package root exposes only the ruled library values and declarations", () => {
  ...
  assert.equal(
    output.trim(),
    "AkumaWorldScopeError,AuthorityCorruptionError,Delivery,Keiyaku,KeiyakuRefused,KeiyakuRetry,NoGitWorldError,Repo,SettingsError,World,WorldError,gatesFrom,settings,worktreeHooksFrom",
  );
});
...
test("Keiyaku owns contract construction over one pinned Repo capability", async () => {
  ...
  const bound = await Keiyaku.bind({ repo, markdown: markdown("Markdown input"), workspace: "worktree" });
  ...
  assert.equal(
    (await Keiyaku.list({ repo })).rows.some((contract) => contract.id === state.id),
    true,
  );
  await assert.rejects(

...
test("a Hook runner outlives its killed reconcile caller and fences immediate replay", async () => {
  ...
  const caller = spawn(process.execPath, [...sourceLoader, "--input-type=module", "-e", callerSource], {
    cwd: process.cwd(),
...
test("reconcile acquires a death-released scratch lock and preserves an actively held lock", async () => {
  ...
  const child = spawn(process.execPath, [...sourceLoader, "--input-type=module", "-e", childSource], {
    cwd: process.cwd(),
    stdio: ["ignore", "ignore", "pipe"],
  });
  let stderr = "";
